...
Zusätzliche Zigbee2MQTT-Instanz für den Koordinator im **Gebäude Saal** (ehemaliger Festsaal) — **ein** netzwerkgebundener Koordinator (SLZB-06p7 über Ethernet/PoE, angesprochen per TCP). Nutzt das **offizielle** Zigbee2MQTT-Add-on-Image `ghcr.io/zigbee2mqtt/zigbee2mqtt-{arch}` — also identische Container wie das offizielle Add-on, nur unter eigenem Slug, damit mehrere Instanzen parallel laufen können.
...
```yaml
data_path: /config/zigbee2mqtt_gebaeudesaal   # eindeutiger Datenpfad je Instanz
mqtt:
  base_topic: zigbee2mqtt_gebaeudesaal        # MUSS je Instanz eindeutig sein
  server: mqtt://core-mosquitto:1883       # bei Mosquitto-Add-on
  user: mqtt_z2m
  password: DEIN_PASSWORT
serial:
  port: tcp://192.0.2.22:6638              # feste IP des SLZB-06p7
...
`ingress` ist aktiv. Über **Einstellungen → Add-ons → dieses Add-on → „In Seitenleiste anzeigen"** erscheint das Z2M-Frontend direkt in der HA-Sidebar — ohne eigenen Port. Der Sidebar-Name entspricht dem Add-on-Namen, daher trägt jede Zone einen sprechenden Namen (hier „… Gebäude Saal").
